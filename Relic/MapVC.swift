...
class MapVC: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
	...
	override func viewDidLoad() {
		...
		userbase.observeSingleEvent(of: .value, with: { snapshot in
			if let model = UserModelObject.getSingleton(snapshot: snapshot){
				...
				self.mapView.setRegion(MKCoordinateRegionMake(model.lastLocation, MKCoordinateSpanMake(self.globalZoom, self.globalZoom)), animated: true)
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		if let dest = segue.destination as? NewRelicVC {
			dest.currentUser = usermodel
		}
		if let dest = segue.destination as? RelicDetailVC, let relic = sender as? RelicModelObject {
			dest.currentUser = usermodel
			dest.currentRelic = relic
			dest.sourceView = "MapVC"
		}
		else {
	...
	@IBAction func saveNewRelic(_ sender: UIStoryboardSegue){
		...
		if let src = sender.source as? NewRelicVC, let relic = src.relic {
	...
	func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
		...
		guard let currentLocation = currentLoc else {
			return nil
		}
		
		let loc1 = CLLocation(coord: annotation.coordinate)
		let loc2 = CLLocation(coord: currentLocation)
	...
	//Update region whenever map region changes
